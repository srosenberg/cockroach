...
func ShouldSplitAtDesc(rawDesc *roachpb.Value) bool {
	__antithesis_instrumentation__.Notify(268376)
	var desc descpb.Descriptor
	if err := rawDesc.GetProto(&desc); err != nil {
		__antithesis_instrumentation__.Notify(268378)
		return false
	} else {
		__antithesis_instrumentation__.Notify(268379)
	}
	__antithesis_instrumentation__.Notify(268377)
	switch t := desc.GetUnion().(type) {
	case *descpb.Descriptor_Table:
		__antithesis_instrumentation__.Notify(268380)
		if t.Table.IsView() && func() bool {
			__antithesis_instrumentation__.Notify(268386)
			return !t.Table.MaterializedView() == true
		}() == true {
			__antithesis_instrumentation__.Notify(268387)
			return false
		} else {
			__antithesis_instrumentation__.Notify(268388)
		}
		__antithesis_instrumentation__.Notify(268381)
		return true
	case *descpb.Descriptor_Database:
		__antithesis_instrumentation__.Notify(268382)
		return false
	case *descpb.Descriptor_Type:
		__antithesis_instrumentation__.Notify(268383)
		return false
	case *descpb.Descriptor_Schema:
		__antithesis_instrumentation__.Notify(268384)
		return false
	default:
		__antithesis_instrumentation__.Notify(268385)
...
func pk(name string) descpb.IndexDescriptor {
	__antithesis_instrumentation__.Notify(268389)
...
func MakeSystemDatabaseDesc() catalog.DatabaseDescriptor {
	__antithesis_instrumentation__.Notify(268390)
...
func systemTable(
	...
	__antithesis_instrumentation__.Notify(268391)
	...
	for _, col := range columns {
		__antithesis_instrumentation__.Notify(268395)
		if tbl.NextColumnID <= col.ID {
			__antithesis_instrumentation__.Notify(268396)
			tbl.NextColumnID = col.ID + 1
		} else {
			__antithesis_instrumentation__.Notify(268397)
		}
	}
	__antithesis_instrumentation__.Notify(268392)
	for _, fam := range families {
		__antithesis_instrumentation__.Notify(268398)
		if tbl.NextFamilyID <= fam.ID {
			__antithesis_instrumentation__.Notify(268399)
			tbl.NextFamilyID = fam.ID + 1
		} else {
			__antithesis_instrumentation__.Notify(268400)
		}
	}
	__antithesis_instrumentation__.Notify(268393)
	for i, idx := range indexes {
		__antithesis_instrumentation__.Notify(268401)
		if tbl.NextIndexID <= idx.ID {
			__antithesis_instrumentation__.Notify(268403)
			tbl.NextIndexID = idx.ID + 1
		} else {
			__antithesis_instrumentation__.Notify(268404)
		}
		__antithesis_instrumentation__.Notify(268402)

		if idx.Unique && func() bool {
			__antithesis_instrumentation__.Notify(268405)
			return i > 1 == true
		}() == true {
			__antithesis_instrumentation__.Notify(268406)
			tbl.Indexes[i-1].ConstraintID = tbl.NextConstraintID
			tbl.NextConstraintID++
		} else {
			__antithesis_instrumentation__.Notify(268407)
		}
	}
	__antithesis_instrumentation__.Notify(268394)
	return tbl
}
...
func registerSystemTable(
	...
	__antithesis_instrumentation__.Notify(268408)
	ctx := context.Background()
	if _, alreadyExists := SystemTableDescriptors[createTableStmt]; alreadyExists {
		__antithesis_instrumentation__.Notify(268412)
		log.Fatalf(ctx, "system table %q cannot be registered, existing entry for %s", tbl.Name, createTableStmt)
	} else {
		__antithesis_instrumentation__.Notify(268413)
	}
	{
		__antithesis_instrumentation__.Notify(268414)
		...
		if privs == nil {
			__antithesis_instrumentation__.Notify(268416)
			log.Fatalf(ctx, "no superuser privileges found when building descriptor of system table %q", tbl.Name)
		} else {
			__antithesis_instrumentation__.Notify(268417)
		}
		__antithesis_instrumentation__.Notify(268415)
		tbl.Privileges = catpb.NewCustomSuperuserPrivilegeDescriptor(privs, security.NodeUserName())
	}
	__antithesis_instrumentation__.Notify(268409)
	for _, fn := range fns {
		__antithesis_instrumentation__.Notify(268418)
		fn(&tbl)
	}
	__antithesis_instrumentation__.Notify(268410)
	b := tabledesc.NewBuilder(&tbl)
	if err := b.RunPostDeserializationChanges(); err != nil {
		__antithesis_instrumentation__.Notify(268419)
		log.Fatalf(
			ctx, "system table %q cannot be registered, error during RunPostDeserializationChanges: %+v",
			tbl.Name, err,
		)
	} else {
		__antithesis_instrumentation__.Notify(268420)
	}
	__antithesis_instrumentation__.Notify(268411)
...
var (
	...
	DescIDSequence = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268421)
...
var (
	...
	CommentsTable = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268422)
			...
			sort.Slice(tbl.Privileges.Users, func(i, j int) bool {
				__antithesis_instrumentation__.Notify(268423)
	...
	ProtectedTimestampsMetaTable = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268424)
	...
	StatementStatisticsTable = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268425)
			tbl.Checks = []*descpb.TableDescriptor_CheckConstraint{{
	...
	TransactionStatisticsTable = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268426)
			tbl.Checks = []*descpb.TableDescriptor_CheckConstraint{{
	...
	SpanConfigurationsTable = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268427)
			tbl.Checks = []*descpb.TableDescriptor_CheckConstraint{{
				Name:      "check_bounds",
	...
	SpanCountTable = registerSystemTable(
		...
		func(tbl *descpb.TableDescriptor) {
			__antithesis_instrumentation__.Notify(268428)
			tbl.Checks = []*descpb.TableDescriptor_CheckConstraint{{
				Name:      "single_row",
				Expr:      "singleton",
				ColumnIDs: []descpb.ColumnID{1},
...
var (
	...
	unleasableSystemDescriptorsByID = func(s []catalog.Descriptor) map[descpb.ID]struct{} {
		__antithesis_instrumentation__.Notify(268429)
		m := make(map[descpb.ID]struct{}, len(s))
		for _, d := range s {
			__antithesis_instrumentation__.Notify(268431)
			m[d.GetID()] = struct{}{}
		}
		__antithesis_instrumentation__.Notify(268430)
		return m
	}(UnleasableSystemDescriptors)

	unleasableSystemDescriptorsByName = func(s []catalog.Descriptor) map[descRefByName]struct{} {
		__antithesis_instrumentation__.Notify(268432)
		m := make(map[descRefByName]struct{}, len(s))
		for _, d := range s {
			__antithesis_instrumentation__.Notify(268434)
			m[descRefByName{
		...
		__antithesis_instrumentation__.Notify(268433)
		return m
	}(UnleasableSystemDescriptors)
)

func IsUnleasableSystemDescriptorByID(id descpb.ID) bool {
	__antithesis_instrumentation__.Notify(268435)
...
func IsUnleasableSystemDescriptorByName(
	...
	__antithesis_instrumentation__.Notify(268436)
