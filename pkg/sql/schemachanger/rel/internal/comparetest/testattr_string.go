package comparetest

import __antithesis_instrumentation__ "antithesis.com/instrumentation/wrappers"

import "strconv"

func _() {
	__antithesis_instrumentation__.Notify(578525)

	var x [1]struct{}
	_ = x[i8-0]
	_ = x[pi8-1]
	_ = x[i16-2]
	_ = x[pi16-3]
	_ = x[i32-4]
	_ = x[pi32-5]
	_ = x[i64-6]
	_ = x[pi64-7]
	_ = x[ui8-8]
	_ = x[pui8-9]
	_ = x[ui16-10]
	_ = x[pui16-11]
	_ = x[ui32-12]
	_ = x[pui32-13]
	_ = x[ui64-14]
	_ = x[pui64-15]
	_ = x[i-16]
	_ = x[pi-17]
	_ = x[ui-18]
	_ = x[pui-19]
	_ = x[str-20]
	_ = x[pstr-21]
	_ = x[_uintptr-22]
	_ = x[puintptr-23]
}

const _testAttr_name = "i8pi8i16pi16i32pi32i64pi64ui8pui8ui16pui16ui32pui32ui64pui64ipiuipuistrpstr_uintptrpuintptr"

var _testAttr_index = [...]uint8{0, 2, 5, 8, 12, 15, 19, 22, 26, 29, 33, 37, 42, 46, 51, 55, 60, 61, 63, 65, 68, 71, 75, 83, 91}

func (i testAttr) String() string {
	__antithesis_instrumentation__.Notify(578526)
	if i < 0 || func() bool {
		__antithesis_instrumentation__.Notify(578528)
		return i >= testAttr(len(_testAttr_index)-1) == true
	}() == true {
		__antithesis_instrumentation__.Notify(578529)
		return "testAttr(" + strconv.FormatInt(int64(i), 10) + ")"
	} else {
		__antithesis_instrumentation__.Notify(578530)
	}
	__antithesis_instrumentation__.Notify(578527)
	return _testAttr_name[_testAttr_index[i]:_testAttr_index[i+1]]
}
