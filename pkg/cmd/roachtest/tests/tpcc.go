...
func setupPrometheusForRoachtest(
	...
	if cfg == nil {
		...
		cfg.WithNodeExporter(c.Range(1, c.Spec().NodeCount).InstallNodes())
		cfg.WithCluster(c.Range(1, c.Spec().NodeCount).InstallNodes())
		if len(workloadInstances) > 0 {
...
func runTPCCPublished(
	...
	if !t.SkipInit() {
		...
		// Initialize and do the import.
		{
			...
			if opts.optimized {
				cmd = tpccImportCmd(opts.warehouses, fmt.Sprintf("--partitions=%d", crdbNodeCount), "--replicate-static-columns", "--partition-strategy=leases", "--checks=false", pgurl)
			} else {
				cmd = tpccImportCmd(opts.warehouses, "--checks=false", pgurl)
			}
		...
		t.L().Printf("Step 6 - This step intentionally left blank")
		{
			...
			if opts.optimized {
				cmd = fmt.Sprintf(
					"./cockroach workload run tpcc  --partitions=%d  --warehouses=%d  --duration=1m --workers=1000 --wait=0.0 {pgurl%s}",
					crdbNodeCount,
					opts.warehouses,
					crdbNodes,
				)
				...
				cmd = fmt.Sprintf(
					"./cockroach workload run tpcc  --warehouses=%d  --duration=1m --workers=1000 --wait=0.0 {pgurl%s}",
					opts.warehouses,
					crdbNodes,
				)
			}
	...
	for _, w := range workers {
		...
		m.Go(func(ctx context.Context) error {
			...
			return c.RunE(ctx, option.WithNodes(c.Node(w.node)), cmd)
		})
	}
	m.Wait()
